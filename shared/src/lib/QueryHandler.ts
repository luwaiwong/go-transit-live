...
/* This project uses Tanstack/react-query for api call handling
  QueryHandler creates the query handler

  use____Query funcitons handle different types of api calls
...
export function useStationsQuery(): ReturnType<typeof useQuery> {
...
export function usePrevious10LaunchesQuery(): ReturnType<typeof useQuery> {
...
export function useLaunchesQuery(type: string, limit: number, offset:number): ReturnType<typeof useQuery> {
  const setUpcomingLaunches = useUserStore((state)=>state.setUpcomingLaunches)
  const setPreviousLaunches = useUserStore((state)=>state.setPreviousLaunches)
...
export function useUpcomingEventsQuery(): ReturnType<typeof useQuery> {
...
export function usePreviousEventsQuery(): ReturnType<typeof useQuery> {
...
export function useNewsQuery(searchparams: string ="") : ReturnType<typeof useInfiniteQuery> {
